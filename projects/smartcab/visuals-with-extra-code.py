...
def calculate_reliability1(data):
	""" Calculates the reliability rating of the smartcab during testing. """
...
def get_ratings():
	print 'calling get ratings'
	df = pd.DataFrame(columns=['filename', 'filesize', 'safety_reliability_rating', 'training_len'])
	dirpath = '..\\smartcab\\smartcab\\logs\\'
	# make a generator for all file paths within dirpath
	all_files = (os.path.join(basedir, filename) for basedir, dirs, files in os.walk(dirpath) for filename in
				 fnmatch.filter(files, 'sim_improved-learning.csv.e*.csv'))
	sorted_files = sorted(all_files, key=os.path.getsize, reverse=True)
	# sorted_files = sorted(all_files, reverse=True)
	# print(all_files)
	for filename in sorted_files:
		# print (filename.replace('smartcab\logs', 'logs'))
		# vs.plot_trials(filename.replace('smartcab\logs', 'logs'))
		data = pd.read_csv(os.path.join("logs", filename.replace('smartcab\logs', 'logs')))
		...
		if len(testing_data) > 0:
			safety_rating = calculate_safety1(testing_data)
			reliability_rating = calculate_reliability1(testing_data)

		# print safety_rating+reliability_rating

		df = df.append(pd.DataFrame([[filename.split('\\')[-1], os.path.getsize(filename), safety_rating+reliability_rating, len(training_data)]], columns = ['filename', 'filesize', 'safety_reliability_rating', 'training_len']))

	df.to_csv('D:\Srikanth I\Training\Py\UC\machine-learning\projects\smartcab\smartcab\logs\\reviewed\consolidated.csv', sep=',', encoding='utf-8')
	return df.sort_values(by=['safety_reliability_rating','filesize'])
